...
import { ArrowLeft, FileText, MessageCircle, Save, Trash2, Phone, Mail, MapPin, Briefcase, Home, BookOpen, Download } from "lucide-react";
...
const STATUS_OPTIONS: { value: RegistrationStatusValue; label: string }[] = [
  { value: "new", label: "Nouveau" },
  { value: "contacted", label: "Contacté" },
  { value: "pending", label: "En attente" },
  { value: "registered", label: "Inscrit" },
  { value: "rejected", label: "Refusé / annulé" },
];

function RegistrationDetail() {
  const { id } = useParams({ from: "/admin/registrations/$id" });
  const { data, isLoading, error } = useRegistrationQuery(id);
  const update = useUpdateRegistrationMutation(id);
  const remove = useDeleteRegistrationMutation(id);
  const confirm = useConfirm();

  const [status, setStatus] = useState<RegistrationStatusValue>("new");
  const [notes, setNotes] = useState("");

  // Sync local form state once data arrives or refetches.
  useEffect(() => {
    if (data) {
  ...
  if (isLoading) {
    return (
      <AdminShell eyebrow="Inscription" title="Chargement…">
        <div className="card-elegant text-sm text-muted-foreground">Chargement de l'inscription…</div>
      </AdminShell>
    );
  }
  ...
  if (error || !data) {
    return (
      <AdminShell eyebrow="Inscription" title="Introuvable">
        <div className="card-elegant text-center py-10">
          <h2 className="font-display text-2xl">Cette inscription n'existe pas</h2>
          <p className="mt-2 text-sm text-muted-foreground">Elle a peut-être été supprimée.</p>
          <Link to="/admin/registrations" className="btn-outline-ink mt-6 inline-flex">Retour à la liste</Link>
        </div>
      </AdminShell>
    );
  }
  ...
  function handleSave() {
    update.mutate(
      ...
      {
        onSuccess: () => toast.success("Inscription mise à jour"),
  ...
  return (
    <AdminShell
      eyebrow={`#${data.id} · soumis le ${formatDateTime(data.submitted_at)}`}
      title={data.full_name}
      actions={
        <Link to="/admin/registrations" className="btn-outline-ink !px-4 !py-2 text-sm inline-flex items-center gap-1.5">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="card-elegant">
            <div className="flex items-start justify-between gap-4 flex-wrap">
              <div>
                ...
                <div className="mt-2 flex items-center gap-2 flex-wrap">
                  ...
                  <span className="text-xs text-muted-foreground">soumis le {formatDateTime(data.submitted_at)}</span>
                </div>
              </div>
              <div className="flex gap-2">
                {data.whatsapp_applicant_url && (
                  <a
                    href={data.whatsapp_applicant_url}
                    target="_blank"
            ...
            <div className="mt-6 grid sm:grid-cols-2 gap-x-6 gap-y-4 text-sm">
              ...
              <InfoRow icon={Home} label="Adresse précise" value={data.address ?? "—"} />
              <InfoRow icon={BookOpen} label="Niveau d'étude" value={data.education_level} />
              <InfoRow icon={Briefcase} label="Profession" value={data.profession ?? "—"} />
            </div>
          </div>

          <div className="card-elegant">
            <h3 className="font-display text-lg">Formation choisie</h3>
            <div className="mt-3 text-sm">
              <div className="font-medium">{data.formation.title ?? "—"}</div>
              {data.formation.detail?.slug && (
                <div className="text-xs text-muted-foreground mt-0.5">/{data.formation.detail.slug}</div>
              )}
            </div>
          </div>
          ...
          {data.message && (
            <div className="card-elegant">
              <h3 className="font-display text-lg">Message du candidat</h3>
          ...
          {data.documents.length > 0 && (
            <div className="card-elegant">
              <h3 className="font-display text-lg">Documents joints</h3>
                ...
                {data.documents.map((doc) => (
                  <li key={doc.id}>
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center gap-3 rounded-xl border px-3 py-2.5 text-sm hover:bg-muted/30 transition-colors"
                      style={{ borderColor: "var(--border)" }}
                    >
                      <FileText size={16} style={{ color: "var(--gold)" }} />
                      <span className="flex-1 truncate">{doc.original_name ?? `Document #${doc.id}`}</span>
        ...
        <aside className="space-y-6">
          <div className="card-elegant">
            ...
            <div className="mt-5">
              ...
              <textarea
                ...
                placeholder="Appelé le 26/05 — laisser un message…"
          ...
          {/* <div className="card-elegant">
            ...
            <dl className="mt-3 space-y-2 text-xs">
              ...
              <Row k="Politique acceptée" v={data.privacy_accepted ? "oui" : "non"} />
            </dl>
          </div> */}
